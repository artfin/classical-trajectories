	...
	bool operator()(std::vector<hep::vegas_result<double>> const& r)
	{
		...
		if ( result.error() < abs_error )
		{
			std::cout << ">> absolute error " << result.error() 
				<< " is smaller than the limit " << abs_error << std::endl;
		...
		if ( result.error() < rel_error * result.value() )
		{
			std::cout << ">> relative error " 
					...
					<< " is smaller than the limit " << rel_error << std::endl;
			// returning false stops all remaining iterations
			return false;
		}
...
int main()
{
	// stop if error is lower than 0.0001 or better than 1%
	hep::vegas_callback<double>(stop_after_precision(0.0001, 0.001));
	
	// print only 3 digits
	std::cout.precision(3);

	// perform 100 iterations with 1000 calls each at maximum
