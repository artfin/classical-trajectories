...
def integrand(x, Temperature):
	# x = [R, theta]
	potential_value = newhutson.extpot(x[0], np.cos(x[1])) * htoj
	if potential_value < 0:
		res = sp.gammainc(2.5, - potential_value / (k * Temperature)) * np.exp(- potential_value / \
			(k * Temperature)) * x[0]**2
		print 'x: {0}; res: {1}'.format(x, res)
	else:
		return 0.0
...
def initialization(T):
	...
	integ = vegas.Integrator([[0., 5.], [0., np.pi]])
...
print newhutson.extpot(0.2, np.cos(np.pi /2))
