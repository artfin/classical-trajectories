...
R_values = [0.1 * i for i in range(10, 50)]
theta = np.pi / 2

parker_values = [potential(R, theta) for R in R_values]
hutson_values = [hutson.extpot(R, np.cos(theta)) for R in R_values]

for v1, v2 in zip(parker_values, hutson_values):
    print 'parker: {0}\t; hutson: {1}'.format(v1, v2)
