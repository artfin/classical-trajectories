...
def plot_spectrum():
    ...
    freqs_class, spectrum_class = read_file( './spectrum_buryak_dipole_woutS.txt' )
    spectrum_class = norm( spectrum_class )

    freqs_d1, spectrum_d1 = read_file( './spectrum_buryak_dipole_woutS_d1.txt' )
    spectrum_d1 = norm( spectrum_d1 )

    freqs_d2, spectrum_d2 = read_file( './spectrum_buryak_dipole_woutS_d2.txt' )
    spectrum_d2 = norm( spectrum_d2 )

    freqs_d3, spectrum_d3 = read_file( './spectrum_buryak_dipole_woutS_d3.txt' )
    spectrum_d3 = norm( spectrum_d3 )

    freqs_bg, spectrum_bg = read_file( '../pictures/bosomworth_gash1965_295K.txt' )

    freqs_withS_d2, spectrum_withS_d2 = read_file( '../output/spectrum_buryak_dipole_d2.txt')
    spectrum_withS_d2 = norm( spectrum_withS_d2 )
    ...
    l1, = plt.plot( freqs_withS_d2, spectrum_withS_d2, color = 'r', linewidth = 2.0, linestyle = '-' )

    fig.legend( (l1, l2, l4), (r'WITH S D2', 'Bosomworth, Gass, 1965', r'WITHOUT S D2'), 'lower center', ncol = 3, fancybox = True, shadow = True )
