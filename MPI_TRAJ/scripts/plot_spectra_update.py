...
lower_bounds_900, higher_bounds_900, contents_900 = read_file( '../experiments/spectrum_lconst_900.txt', n = 3 )
...
means_900 = [ ( l + h ) * 0.5 for l, h in zip( lower_bounds_900, higher_bounds_900) ]
...
alpha_900 = [ omega**2 * j * (1 - np.exp( - omega * planck_constant / ( k * temperature ))) for omega, j in zip( means_900, contents_900) ]
...
patch_yellow = mpatches.Patch( color = 'y', label = r'$L^2$ = 900' )
...
plt.plot( means_900[begin:end], contents_900[begin:end], linewidth = lw, color = 'y' )
...
#plt.plot( means_900[begin:end], alpha_900[begin:end], linewidth = lw, color = 'y' )

plt.grid( linestyle = ':', alpha = 0.7 )
plt.legend( handles = [patch_red, patch_green, patch_blue, patch_yellow] )
plt.show()
