...
void master_code( int world_size )
{
	...
	FILE* inputfile = fopen("input/DIATOM/visualize", "r" );
	//FILE* inputfile = fopen("input/DIATOM/test2", "r" );

	string spectrum_filename = "visualize";
...
void save_coordinates( vector<tuple<double, double, double>> &he_coords, 
					   vector<tuple<double, double, double>> &ar_coords,
					   string filename )
{
	ofstream file( filename );

	for ( int counter = 0; counter < he_coords.size(); counter++ )
	{
		file << get<0>( he_coords[counter] ) << " " <<
				get<1>( he_coords[counter] ) << " " << 
				get<2>( he_coords[counter] ) << " " <<
				get<0>( ar_coords[counter] ) << " " << 
				get<1>( ar_coords[counter] ) << " " << 
				get<2>( ar_coords[counter] ) << endl;
	}
...
void slave_code( int world_rank )
{
	...
	while ( true )
	{
		...
		while ( y0[0] < end_value ) 
		{
			...
			transform_coordinates( he_temp, ar_temp, y0[0], y0[2] );
		...
		save_coordinates( he_coords, ar_coords, "coords.txt" );
