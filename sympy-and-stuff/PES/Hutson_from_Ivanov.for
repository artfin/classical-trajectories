	...
	1RDISPC,RREF,BFAC,RDISPO,BOND,C8O2,ISITE,C10O,C8O,C6O,C10C2,C8C2,
     ...
     3NLEGC,NLEGO
C       Initialize intermolecular parameters
	CALL POTDAT4   ! CO2-Ar Hutson potential (the best!)
C
C	Input R and cos(teta)
C
        R=12.D0 ! Intermolecular distance, Angstroms
	COSTH=0.3D0 ! Cosine of "teta" ("gamma" in my code)
	VPOT=EXTPOT(R,COSTH)
	WRITE(*,*) R,COSTH,VPOT
	STOP
	END
C-------------------------------------------------------
	SUBROUTINE POTDAT4
C~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
C  Set the the parameters of interaction potential 
C  CO2-Ar anisotropic Hutson (the best!)
C   SPLIT REPULSION POTENTIAL (1 minimum) (parameter ISITE=1)
C (J.Chem.Phys.,1996,V.105(20),9130-9140) 
C~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	IMPLICIT REAL*8 (A-H,O-Z)
      COMMON/POTHUT/ RLAM(20),BLAM(20),OLAM(20),
	1RDISPC,RREF,BFAC,RDISPO,BOND,C8O2,ISITE,C10O,C8O,C6O,C10C2,C8C2,
...
C Evaluates Ar-CO2 Hutson-Roche potential for single value 
C of intermolecular distance R and COSINE THETA (COSTH) 
C (i.e., for our's cosine of angle "gamma").
C R is in Angstroms, final result is in Tsukanov's units (E-14 erg),
C but the internal workings are in atomic units.
C !!! Parameters of potential must be at first initialised by POTDAT4 !!!
	IMPLICIT REAL*8 (A-H,O-Z)
      COMMON/POTHUT/ RLAM(20),BLAM(20),OLAM(20),
	1RDISPC,RREF,BFAC,RDISPO,BOND,C8O2,ISITE,C10O,C8O,C6O,C10C2,C8C2,
     2C6C2,C10C,C8C,C6C,SITE,C10O2,SFAC2,SFAC0,C6O2,SFACE,SFACC,
	...
	BC=1.38066D-2 ! Boltzmann constant 
C !!!!  THE LONG-RANGE PARAMETERS must be previously INITIALISED
      RR=R/AUR
      IF(ISITE.GT.0) THEN	 ! Split repulsion potential 
        T=-BOND/RR
...
C
     ...
     1   +(C6O2+(C8O2+C10O2*R2)*R2)*(1.5D0*COSO2*COSO2-0.5D0))*R2**3
      ...
      ENDIF
C
      EXTPOT=V*AUE ! Scale back to cm-1
      EXTPOT=EXTPOT*1.4388*BC ! Convert energy to Tsukanov's units
      RETURN
	END
C--------------------------------------------------------------------
